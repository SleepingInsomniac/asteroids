...
class Asteroids < PF::Game
  ...
  def initialize(*args, **kwargs)
    ...
    @ship = Ship.new
    @ship.position = PF2d::Vec2.new(x: width / 2.0, y: height / 2.0)
  ...
  def generate_asteroids
    center_size = PF2d::Vec2.new(50.0, 50.0)
    center_pos = PF2d::Vec2.new((width.to_f / 2.0) - (center_size.x / 2.0), (height.to_f / 2.0) - (center_size.y / 2.0))

    @asteroid_count.times do
      a = Asteroid.new
      loop do
        a.position = PF2d::Vec2.new(x: rand(0.0..width.to_f), y: rand(0.0..height.to_f))
      ...
      v_max = 30.0
      a.velocity = PF2d::Vec2.new(x: rand(-v_max..v_max), y: rand(-v_max..v_max))
  ...
  def setup_round
    ...
    @ship.blew_up = false
    @ship.position = PF2d::Vec2.new(x: width / 2.0, y: height / 2.0)
    @ship.velocity = PF2d::Vec2.new(0.0, 0.0)
    @ship.rotation_speed = 0.0
  end

  def wrap(position : PF2d::Vec2)
  ...
  def update(dt : Float64)
    ...
    @ship.rotate_right(dt) if @controller.held?("Rotate Right")
    @ship.rotate_left(dt) if @controller.held?("Rotate Left")
    @ship.thrust(dt) if @controller.held?("Thrust")
    @ship.update(dt)

    @ship.position = wrap(@ship.position)

    if @controller.held?("Fire") && @ship.can_fire?
      @bullets << @ship.fire
    end
      ...
      @asteroids.each do |asteroid|
        if bullet.collides_with?(asteroid)
          if asteroid.mass > 7.0
            2.times do
              a = Asteroid.new
              a.position = asteroid.position + PF2d::Vec2.new(rand(-1.0..1.0), rand(-1.0..1.0))
              v_max = 30.0
              a.velocity = PF2d::Vec2.new(x: rand(-v_max..v_max), y: rand(-v_max..v_max))
              a.rotation_speed = rand(-5.0..5.0)
